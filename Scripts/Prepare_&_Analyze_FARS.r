...
#--------------------------
	...
	Fc_desc_87_14.. <- data.frame(Fc_desc = c("Interstate, principle arterial","Interstate, principle arterial","Freeway, expressway, principle arterial","Principle arterial, other",
		...
		Fc_code = c(1,11,12,2,13,3,14,4,5,15,6,16,9,19,99))
	#2015-current
...
#-------------------------
	...
	All_State_Fc_Desc_Data.. <- data.frame()
	...
	for(year in Years.){
		...
		if(nrow(Temp_State..) > 0){		
			...
			#Store in data frame
			All_State_Fc_Desc_Data.. <- rbind(All_State_Fc_Desc_Data.., Temp_State..) 
		}
		...
		if(nrow(Temp_State..) > 0){
			...
			Temp_State..$Year <- year
			#Rearrange columns
			Temp_State.. <- Temp_State..[,colnames(State_Data..)]
	...
	save(All_State_Fc_Desc_Data..,file =  paste(getwd(), "/Data/Processed_Data/State_Fatal_Data.RData", sep=""))
...
#----------------------------
	...
	sum(State_Data..$Count[State_Data..$Year%in%"2016" & State_Data..$Measure%in%"All Fatals"])
	#Total ped traffic deaths - 2016
	sum(State_Data..$Count[State_Data..$Year%in%"2016" & State_Data..$Measure%in%"Pedestrian Fatals"])
...
#-----------------------
	...
	#Reorder columns
	National_Data.. <- National_Data..[,colnames(State_Data..)]
	#Add to All State Data
	State_Data.. <- rbind(State_Data.., National_Data..)
	...
	State_Data..$Count_Change_Base <- (State_Data..$Count / State_Data..$Base_Count) - 1
